...
function sendRequest(activitesUtilisateur) {
  ...
  const requestData = JSON.stringify({ activitesUtilisateur }); //convertire en chaine json
  ...
  jQuery.ajax({
    ...
    success: function(response) {
      if (response.success) {
        ...
        // Use the activities data as needed
      } else {
...
// Fonction pour soumettre une requête à Neo4j

//const neo4j_http_url = "http://localhost:8080/wp-json/custom/v1/neo4j-query";


async function getNeo4jConfig() {
	console.log("ystroppa1");
    const response = await fetch('/wp-json/custom/v1/neo4j-config');
    const config = await response.json();
    return config;
}
	console.log("ystroppa1");

/*getNeo4jConfig().then(config => {
	
	console.log("ystroppa2");
	console.log(config.url);
	console.log(config.username);
	console.log(config.password);
	
    const neo4j_http_url = config.url;
    const neo4jUsername = config.username;
            ...
            inchikeyItems.forEach((item) => {
              ...
              item.addEventListener("click", () => {
                const clickedInchikeyElement =
                  item.querySelector("p:first-child"); // Sélectionne le premier élément <p> dans le <li> pour obtenir l'InChIKey
                const clickedInchikey =
                  clickedInchikeyElement.textContent.replace("InCh: ", ""); // Obtient l'InChIKey à partir du texte du premier <p>
                const clickedMolecule = molecules.find(
                  (molecule) => molecule.properties.inchikey === clickedInchikey
                );
                if (clickedMolecule) {
                  const clickedSmileElement =
                    item.querySelector("p:nth-child(2)"); // Sélectionne le deuxième élément <p> dans le <li> pour obtenir le SMILE
                  const clickedSmile = clickedSmileElement.textContent.replace(
                    "Smile: ",
                    ""
                  ); // Obtient le SMILE à partir du texte du deuxième <p>

                  // Log the clicked InChIKey and SMILES to the corresponding DLMOL object
                  const currentRequete =
                    activitesUtilisateur.requetes[
                      activitesUtilisateur.requetes.length - 1
                    ];
...
//saveUserActivities(activitesUtilisateur);
  ...
  /*const historyDiv = document.getElementById('history-div');
	...
	activitesUtilisateur.requetes.forEach(requete => {
		const li = document.createElement('li');
		const timestamp = new Date().toLocaleString(); 

		//const requestText = `${requete.requeteCypher.requete} - ${timestamp}`;
		const requestText = `${requete.requeteCypher.requete}`;	
  ...
  /*const updatedActivitesUtilisateur = activitesUtilisateur;

	// Envoyer l'objet mis à jour au serveur en utilisant AJAX
	jQuery.ajax({
		url: '/wp-admin/admin-ajax.php',
		type: 'POST',
		data: {
			action: 'store_user_activities', // Nom de l'action AJAX
			activitesUtilisateur: JSON.stringify(updatedActivitesUtilisateur) // Convertir l'objet en chaîne JSON
		},
		success: function(response) {
			console.log('Les activités des utilisateurs ont été enregistrées dans la session :', response);
		},
		error: function(error) {
			console.error('Erreur lors de l\'enregistrement des activités des utilisateurs :', error);
		}
	});

	jQuery(document).ready(function($) {
		// Récupérer les activités utilisateur lors du chargement de la page
		$.ajax({
			url: '/wp-admin/admin-ajax.php',
			type: 'POST',
			data: {
				action: 'get_user_activities' // Nom de l'action AJAX
			},
			success: function(response) {
				if (response.success) {
					const activitesUtilisateur = response.data;
					const historyDiv = document.getElementById('history-div');
					const ul = historyDiv.querySelector('ul');
					ul.innerHTML = '';
					...
					activitesUtilisateur.requetes.forEach(requete => {
						const li = document.createElement('li');
						const date = new Date(requete.timestamp).toLocaleString();
						const requestText = `${requete.requeteCypher.requete} - ${date}`;
						li.textContent = requestText;
						ul.appendChild(li);
					});
					...
					console.error('Erreur lors de la récupération des activités des utilisateurs :', response.data);
				}
			},
			error: function(error) {
				console.error('Erreur lors de la récupération des activités des utilisateurs :', error);
			}
		});
	});
	...
	const activitesJSONS = JSON.stringify(activitesUtilisateur);

	localStorage.setItem('activitesUtilisateur', activitesJSONS);
	console.log("localStorage",localStorage);
	const activitesJSONR = localStorage.getItem('activitesUtilisateur');
	const activitesUser = JSON.parse(activitesJSONR);
	...
	activitesUser.requetes.forEach(requete => {
		const li = document.createElement('li');
		const timestamp = new Date().toLocaleString(); 
